...
void callback_mouse(int button, int state, int x, int y) //нажатие мыши
{
	auto pSpider = remove_if(listSpider.begin(), listSpider.end(),
		[x, y](const Spider& spider) {
			return (fabs(spider.x - x) <= bodySize
				&& fabs(spider.y + y) <= bodySize); });
	...
	if (state == GLUT_DOWN && button == GLUT_LEFT_BUTTON)
	{
		...
		switch (rnd)
		{
		case 1: r = rand() % 180; g = rand() % 180; b = rand() % 180;
			break;
		case 2: r = rand() % 180; g = rand() % 180; b = 180;
			break;
		case 3: r = rand() % 180; g = 180; b = rand() % 180;
			break;
		}
